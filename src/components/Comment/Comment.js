...
function Comment({ type, fetchParentComments, data, type_id, onClose }) {
    ...
    const fetchChildrenComments = async (parent_id) => {
        setShowRepliesComment(true);

        const response = await api.get(userApis.getChildrenComments(type, type_id, parent_id));
            ...
            <div className={cx('wrapper')}>
                ...
                <div className={cx('body')}>
                    ...
                    {numberRepliesComment > 0 && !showRepliesComment && (
                        <div className={cx('view-more')} onClick={() => fetchChildrenComments(data._id)}>
                            ...
                            <div className={cx('replies-content')}>
                                {childrenComments?.length > 0 &&
                                    childrenComments.map((comment, index) => {
                                        return (
                                            <div key={index}>
                                                <ReplyComment type={type} type_id={type_id} data={comment} />
                                            </div>
                                        );
                                    })}
                            </div>
